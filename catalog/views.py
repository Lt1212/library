...
# 续借
@login_required
@permission_required('catalog.renew_due_back')
...
@login_required
def mybooks(request):
    bookinst_list=BookInstance.objects.filter(status__exact='o').filter(borrower=request.user).order_by('due_back')
    if request.method == 'POST':
        bookinst_id=request.POST.get("bookinst_id")
        bookinst=BookInstance.objects.filter(id=bookinst_id)
        bookinst.update(status='a')
    return render(
        request,
